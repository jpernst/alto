...
impl Alto {
	...
	pub fn open(&self, spec: Option<&CStr>) -> AlcResult<Device> {
		let dev = if let Some(spec) = spec {
			unsafe { self.api.owner().alcOpenDevice()(spec.as_ptr()) }
		} else {
			unsafe { self.api.owner().alcOpenDevice()(ptr::null()) }
		};
		...
		if dev == ptr::null_mut() {
			...
			Ok(Device{alto: self, dev: dev, exts: unsafe { ext::AlcCache::new(self.api.owner(), dev) }})
		}
	}
	...
	pub fn open_loopback(&self, spec: Option<&CStr>) -> AlcResult<LoopbackDevice> {
		self.api.rent(|exts| {
			...
			if dev == ptr::null_mut() {
				...
				Ok(LoopbackDevice{alto: self, dev: dev, exts: unsafe { ext::AlcCache::new(self.api.owner(), dev) }})
			}
		})
	}
	...
	pub fn open_capture(&self, spec: Option<&CStr>, freq: sys::ALCuint, format: StandardFormat, size: sys::ALCsizei) -> AlcResult<CaptureDevice> {
		let dev = if let Some(spec) = spec {
			unsafe { self.api.owner().alcCaptureOpenDevice()(spec.as_ptr(), freq, format.into_raw(), size) }
		} else {
			unsafe { self.api.owner().alcCaptureOpenDevice()(ptr::null(), freq, format.into_raw(), size) }
		};
		self.get_error(ptr::null_mut())?;
...
impl<'a> CaptureDevice<'a> {
	...
	#[inline(always)]
	pub fn raw_dev(&self) -> *mut sys::ALCdevice { self.dev }
}
